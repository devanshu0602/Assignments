...
def FCFS(n, arrival_time, burst_time):
    ...
    print_values(arrival_time, burst_time, completion_time,turn_around_time, waiting_time)
    # Find the average TAT and WT
    avg_TAT = find_average(turn_around_time)
    print("\nThe average TurnAroundTime = " + str(avg_TAT))
    avg_WT = find_average(waiting_time)
    print("The average WaitingTime = " + str(avg_WT))
...
for i in range(0, n):
    print("Process[" + str(i + 1) + "]: " + 
          str(arrival_time[i]) + " | " +
          str(burst_time[i]))
# Sorting the table for execution of the
